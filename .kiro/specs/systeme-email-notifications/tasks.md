...
- [ ] Migration créée avec `npx prisma migrate dev --name add_email_content_fields`
- [ ] Champs ajoutés : `htmlBody`, `textBody`, `attachments`, `lastAttemptAt`, `nextRetryAt`
- [ ] Champ `userId` rendu nullable
- [ ] Ancien champ `lastAttemptAt` renommé en `nextRetryAt`
- [ ] Index créé : `idx_emaildelivery_status_nextretryat`
...
**Critères d'acceptation** :
- [ ] Index créé : `idx_emaildelivery_userid_createdat`
- [ ] Index créé : `idx_emaildelivery_createdat_status`
- [ ] Index testés avec EXPLAIN ANALYZE
...
**Critères d'acceptation** :
- [ ] `node-cron` installé : `npm install node-cron`
- [ ] Types installés : `npm install --save-dev @types/node-cron`
- [ ] Version ajoutée au `package.json`
...
**Critères d'acceptation** :
- [ ] `winston` installé : `npm install winston`
- [ ] Configuration de base créée dans `src/config/logger.js`
- [ ] Logs écrits dans `logs/email-error.log` et `logs/email-combined.log`
...
**Critères d'acceptation** :
- [ ] Méthode `createEmail()` créée avec persistance de `htmlBody`, `textBody`, `attachments`
- [ ] Méthode `envoyerEmailConfirmation()` mise à jour (userId=null, textBody ajouté)
- [ ] Méthode `envoyerEmailBienvenue()` mise à jour (userId ajouté, textBody ajouté)
- [ ] Méthode `envoyerEmailPreInscription()` mise à jour (userId ajouté, textBody ajouté)
- [ ] Méthode `envoyerEmailConvocation()` mise à jour (userId ajouté, textBody ajouté)
- [ ] Méthode `envoyerEmailRejet()` mise à jour (userId ajouté, textBody ajouté)
- [ ] Méthode `envoyerEmailSousReserve()` mise à jour (userId ajouté, textBody ajouté)
- [ ] Validation des paramètres implémentée
- [ ] Rate limiting conditionnel (si userId fourni)
- [ ] Retour immédiat avec `{ emailId, status: 'QUEUED' }`
...
**Critères d'acceptation** :
- [ ] Fonction `validateEmail(email)` créée avec regex
- [ ] Fonction `validateParams(params)` créée
- [ ] Fonction `sanitizeInput(input)` créée pour prévenir XSS
...
**Critères d'acceptation** :
- [ ] Classe `RateLimiter` créée
- [ ] Méthode `checkRateLimit(userId)` implémentée
- [ ] Limite par utilisateur : 10 emails/heure
- [ ] Limite globale : 100 emails/heure
- [ ] Alerte créée si queue > 500 emails
- [ ] Tests unitaires
...
**Critères d'acceptation** :
- [ ] Classe `EmailWorker` créée
- [ ] Cron job configuré : toutes les 10 secondes
- [ ] Méthode `start()` implémentée
- [ ] Méthode `stop()` implémentée (graceful shutdown)
- [ ] Méthode `processQueue()` implémentée (max 5 emails)
- [ ] Méthode `processEmail()` implémentée avec lock optimiste
- [ ] Méthode `calculateNextRetry()` implémentée (exponential backoff)
- [ ] Gestion des pièces jointes
- [ ] Création d'alertes en cas d'échec définitif
- [ ] Tests unitaires
...
**Critères d'acceptation** :
- [ ] Worker importé dans `server.js`
- [ ] `emailWorker.start()` appelé au démarrage
- [ ] `emailWorker.stop()` appelé sur SIGTERM
- [ ] `emailWorker.stop()` appelé sur SIGINT
...
**Critères d'acceptation** :
- [ ] Configuration Winston créée dans `src/config/logger.js`
- [ ] Logs error dans `logs/email-error.log`
- [ ] Logs combined dans `logs/email-combined.log`
- [ ] Format JSON avec timestamp
- [ ] Fonction `maskEmail()` pour masquer les emails dans les logs
