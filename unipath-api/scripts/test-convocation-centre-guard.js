...
async function main() {
  ...
  try {
    ...
    const inputFiles = fs.readdirSync(require('path').join(__dirname, '../temp'))
      .filter((f) => f.startsWith('input-convocation-'));
    const lastInput = inputFiles.sort().pop();
    if (lastInput) {
      const payload = JSON.parse(
        fs.readFileSync(require('path').join(__dirname, '../temp', lastInput), 'utf8'),
      );
