...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'MembreCommission'
  ) THEN
...
-- Créer la table DossierInscription de manière idempotente
...
BEGIN
  IF EXISTS (
    ...
    WHERE NOT EXISTS (
      ...
      WHERE "DossierInscription"."inscriptionId" = "Inscription"."id"
    );
...
-- ActionHistory: rendre la migration tolérante si table absente
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'ActionHistory'
  ) THEN
    ...
    IF EXISTS (
      ...
      WHERE table_schema = 'public' AND table_name = 'ActionHistory' AND column_name = 'dossierId'
    ) THEN
      UPDATE "ActionHistory"
      SET "dossierInscriptionId" = (
        SELECT "id"
        FROM "DossierInscription"
        WHERE "DossierInscription"."inscriptionId" = "ActionHistory"."dossierId"
      )
