...
export default function DGESAnneesAcademiques() {
  ...
  const charger = async () => {
    ...
    setError('');
    try {
      const data = await dgesService.listerAnneesAcademiques();
  ...
  const handleCreate = async (e) => {
    ...
    setMessage('');
    try {
      const res = await dgesService.creerAnneeAcademique({
  ...
  const handleDefinirEnCours = async (annee) => {
    ...
    if (
      !window.confirm(
        `Définir « ${annee.libelle} » comme année académique en cours (Module 2 — établissements privés) ?\n`
          + 'Les candidatures et campagnes des étudiants seront rattachées à cette année.'
      )
    ) {
      return;
    }
    ...
    setMessage('');
    try {
      const res = await dgesService.definirAnneeEnCours(annee.id);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Module 2 — établissements privés. L&apos;année en cours définie ici s&apos;applique aux
            campagnes d&apos;inscription, préinscriptions et dossiers des étudiants dans ce module.
            Elle est indépendante de l&apos;année gérée par la DEC pour les concours (Module 1).
          </p>
        </div>

        {anneeEnCours && (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            Année académique en cours (Module 2) : <strong>{anneeEnCours.libelle}</strong>
          </div>
        )}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-left px-6 py-3 font-semibold text-gray-600">Statut Module 2</th>
